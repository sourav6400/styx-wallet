...
class WalletController extends Controller
{
    ...
    public function send_token(Request $request, BalanceService $balanceService)
    {
        ...
        try {
            ...
            switch ($tokenName) {
                case 'Bitcoin':
                    $response = $this->sendUtxoTransaction($chain, $senderAddress, $privateKey, $receiverAddress, $amount, '0.000003');
                    break;
                case 'Litecoin':
                    $response = $this->sendUtxoTransaction($chain, $senderAddress, $privateKey, $receiverAddress, $amount, '0.0002');
                    break;
                case 'Dogecoin':
                    $response = $this->sendUtxoTransaction($chain, $senderAddress, $privateKey, $receiverAddress, $amount, '0.00007');
                    break;
                case 'BNB':
                    $response = $this->sendBscTransaction($privateKey, $receiverAddress, $amount);
                    break;
                case 'Tether':
                    $response = $this->sendTronTransaction($privateKey, $receiverAddress, $amount);
                    break;
                case 'Tron':
                    $response = $this->sendTronTransaction($privateKey, $receiverAddress, $amount);
                    break;
                case 'Ethereum':
                    $response = $this->sendEthereumTokenTransaction($privateKey, $receiverAddress, $amount);
                    break;
                default:
                    Log::error("No endpoint configured for token: {$tokenName}");
                    $message = 'Transaction method not configured for this token';
                    $response = null;
            }

            if ($response) {
                [$status, $message, $details] = $this->processTransactionResponse($response, $tokenName, $userId);
            }
        } catch (\Throwable $e) {
            $status = 'error';
    ...
    /**
     * Send UTXO-based transaction (Bitcoin, Litecoin, Dogecoin)
     */
    private function sendUtxoTransaction($chain, $senderAddress, $privateKey, $receiverAddress, $amount, $fee)
    {
     ...
     */
    private function sendBscTransaction($privateKey, $receiverAddress, $amount)
    {
        return Http::timeout(30)
            ->retry(2, 1000)
            ->withHeaders([
    ...
    /**
     * Send Tron transaction (for both TRX and USDT)
     */
    private function sendTronTransaction($privateKey, $receiverAddress, $amount)
    {
        return Http::timeout(30)
            ->retry(2, 1000)
            ->withHeaders([
    ...
    /**
     * Send Ethereum token transaction
     */
    private function sendEthereumTokenTransaction($privateKey, $receiverAddress, $amount)
    {
        return Http::timeout(30)
            ->retry(2, 1000)
            ->withHeaders([
    ...
    /**
     * Process transaction response and return status, message, details
     */
    private function processTransactionResponse($response, $tokenName, $userId)
    {
        $data = $response->json();

        if (isset($data['txId'])) {
            return ['success', $data['txId'], ''];
        }

        $message = $data['message'] ?? 'Transaction failed';
        $details = $data['error'] ?? 'Unknown error';

        Log::error("{$tokenName} transaction failed for user {$userId}: " . json_encode($data));
