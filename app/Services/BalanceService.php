...
class BalanceService
{
    public function getFakeBalance()
    {
        ...
        $cacheKey = "fake_balance_{$user_id}";
    ...
    {
        $user_id = Auth::user()->id;
        $cacheKey = "filtered_tokens_{$user_id}";
        ...
        return Cache::remember($cacheKey, 3600, function () use ($user_id) {
            ...
            foreach ($filtered as $key => $value) {
                $totalUsd = $totalUsd + $value['tokenBalance'] * $value['usdUnitPrice'];
                $totalCrypto = $totalCrypto + $value['tokenBalance'];
            }
            
            // Return tokens as array with associative keys and totals in separate keys
            return [
                'tokens' => $filtered,
                'totalUsd' => $totalUsd,
